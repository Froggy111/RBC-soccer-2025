...
class MotionController {
  public:
    ...
    void init(float rotation_kp, float rotation_ki, float rotation_kd,
              float velocity_kp, float velocity_ki, float velocity_kd,
              float pf);
    ...
    std::tuple<float, float> rotation_matrix(std::tuple<float, float> vec,
                                             float angle);
    ...
    std::tuple<float, float, float, float> velocity_pid(float current_heading,
    ...
    position_pid(std::tuple<float, float> target_position,
  ...
  private:
    ...
    std::deque<float> velocity_x_errors =
        std::deque<float>(ERROR_WINDOW_SIZE, 0.0);
    std::deque<float> velocity_y_errors =
        std::deque<float>(ERROR_WINDOW_SIZE, 0.0);

    //Sliding Window of errors in rotation
    std::deque<float> rotation_errors =
        std::deque<float>(ROTATION_ERRORS_WINDOW_SIZE, 0.0);

    //Sliding Window of masured velocities
    std::deque<float> x_velocities =
        std::deque<float>(VELOCITY_WINDOW_SIZE, 0.0);
