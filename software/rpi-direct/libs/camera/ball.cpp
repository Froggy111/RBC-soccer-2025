...
bool BallDetector::detectBall(const cv::Mat& frame, cv::Point& ballPosition, double& ballHeading)
{
    cv::Mat irMask;
...
std::vector<IRPoint> BallDetector::detectIRPoints(const cv::Mat& frame, cv::Mat& irMask)
{
    ...
    cv::findContours(irMask.clone(), contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
...
double BallDetector::calculateDistance(const cv::Point& p1, const cv::Point& p2)
{
...
cv::Point BallDetector::findStrongestIRSource(const std::vector<IRPoint>& irPoints)
{
    if (irPoints.empty()) {
    ...
    auto maxPoint = std::max_element(irPoints.begin(), irPoints.end(),
        [](const IRPoint& a, const IRPoint& b) { return a.intensity < b.intensity; });
    
    return maxPoint->position;
}

void BallDetector::drawDebugInfo(cv::Mat& outputFrame, const std::vector<IRPoint>& points, const cv::Point* strongestPoint)
{
    // Mark center point
    ...
    for (const auto& point : points) {
        ...
        // Draw circle
        cv::circle(outputFrame, point.position, radius, cv::Scalar(255, 0, 255), -1);
        
        // Draw line from center to point
        cv::line(outputFrame, m_centerPoint, point.position, cv::Scalar(255, 0, 255), 2);
        
        // Display information
        cv::putText(outputFrame, 
                   "(" + std::to_string(point.position.x) + "," + std::to_string(point.position.y) + ")",
                   cv::Point(point.position.x - 40, point.position.y - 15),
                   cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 255), 1);
                   
        cv::putText(outputFrame,
                   "Angle: " + std::to_string(static_cast<int>(point.angle * 10) / 10.0) + "°",
                   cv::Point(point.position.x - 40, point.position.y + 15),
                   cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 255), 1);
                   
        cv::putText(outputFrame,
                   "Dist: " + std::to_string(static_cast<int>(point.distance * 10) / 10.0),
    ...
    if (strongestPoint != nullptr && strongestPoint->x >= 0) {
        cv::circle(outputFrame, *strongestPoint, 15, cv::Scalar(0, 255, 255), 2);
        cv::putText(outputFrame,
                   "Strongest IR",
...
cv::Mat BallDetector::createDebugView(const cv::Mat& frame, const cv::Mat& irMask, 
                                     const std::vector<IRPoint>& points, const cv::Point* strongestPoint)
{
    // Create output frame
    ...
    if (strongestPoint != nullptr && strongestPoint->x >= 0) {
        ...
        for (const auto& point : points) {
            if (point.position == *strongestPoint) {
                cv::putText(infoDisplay,
                           "Strongest IR Angle: " + std::to_string(static_cast<int>(point.angle * 10) / 10.0) + "°",
                           cv::Point(10, 70),
                           cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
                           
                cv::putText(infoDisplay,
                           "Strongest IR Distance: " + std::to_string(static_cast<int>(point.distance * 10) / 10.0),
                           cv::Point(10, 110),
                           cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 255, 0), 2);
                break;
            }
    ...
    cv::hconcat(std::vector<cv::Mat>{irDisplayResized, infoDisplayResized}, debugTop);
