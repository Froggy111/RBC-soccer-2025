...
int main(int argc, char** argv) {
    ...
    for (int angle = 0; angle < num_frames; angle++) {
        std::cout << "Generating heatmap for heading " << angle << " degrees..." << std::endl;
        
        // Convert angle to radians
        float heading_rad = angle * M_PI / 180.0f;
        
        // Create empty image for the heatmap
        cv::Mat heatmap(field_height, field_width, CV_8UC3, cv::Scalar(0, 0, 0));
        ...
        for (int y = 0; y < field_height; y++) {
            for (int x = 0; x < field_width; x++) {
                ...
                heatmap.at<cv::Vec3b>(y, x) = cv::Vec3b(blue, 0, red);
            }
        ...
        filename << "heatmap_frames/frame_" << std::setw(3) << std::setfill('0') << angle << ".png";
    ...
    std::cout << "All heatmaps generated in " << total_duration << " seconds." << std::endl;
    
    // Create a video from the frames
    std::cout << "Creating video from frames..." << std::endl;
    std::string ffmpeg_cmd = "ffmpeg -y -framerate 30 -i heatmap_frames/frame_%03d.png -c:v libx264 -pix_fmt yuv420p heatmap_video.mp4";
    int result = system(ffmpeg_cmd.c_str());
    
    if (result == 0) {
        std::cout << "Video created successfully: heatmap_video.mp4" << std::endl;
    } else {
