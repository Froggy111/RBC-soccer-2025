...
void loop() {
  Result<std::string, Status> input_wrapped = debug::read_input();
  if (input_wrapped.status == Status::ok) {
    std::string input = input_wrapped.value;
    if (input == "reset") {
      ...
      do {
        ...
        if (liftoff_calibration_status == PMW3360::LiftoffCalibrationStatus::min_length_met) {
          debug::printf(".");
          if (debug::read_input().status == Status::ok) {
            debug::printf("\n");
            break;
          }
        }
        else if (debug::read_input().value == "cancel") {
    ...
    Recommended minimum surface quality: %u. Recommended raw data threshold: %u\n",
    ...
    else if (input == "set_liftoff_zero") {
      sensor.set_minimum_squal(0x00);
      sensor.set_raw_data_threshold(0x00);
    }
    else if (input == "get_liftoff_zero") {
      Result<u8, Status> original_min_squal = sensor.get_minimum_squal();
      if (original_min_squal.status != Status::ok) {
        debug::crash("Error reading minimum surface quality.");
      }
      ...
      debug::printf("Current minimum surface quality: %u. Current raw data threshold: %u\n",
        original_min_squal.value, original_raw_data_threshold.value);
    }
    else {
      debug::printf("ERROR: Invalid command: %s\n", input.c_str());
    }
  }
